...
test("If I provide an invalid token, I get an error", async ({ page }) => {
  await page.goto("/");
  await page.getByLabel("Token").fill("no-way-jose");
  await page.getByRole("button", { name: "Login" }).click();
...
test("If I provide an expired token, I'm asked to login", async ({ page }) => {
  await page.goto("/");
  await page.getByLabel("Token").fill(expiredAccessToken);
  await page.getByRole("button", { name: "Login" }).click();
  await page.getByText("Invalid token");
});
